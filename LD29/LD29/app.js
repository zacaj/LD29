...
var Entity = (function () {
    ...
    Entity.prototype.collide = function (e) {
        ...
        if (dx * dx + dy * dy + dz * dz < e.r * e.r + this.r * this.r)
            return true;
        return false;
    };
...
var Player = (function (_super) {
    ...
    Player.prototype.die = function () {
        ...
        document.getElementById('content').innerHTML = "game over :(<br><br><input type='text' name='name' id='name' value='" + getItem('name') + "'></input><br><button type='button' onclick='submitScore()'>Submit score</button><br>" + '<br><br><input type="button" value="Restart" onClick="window.location.reload()">';
        paused = true;
    };
    return Player;
})(Entity);
...
function update() {
    ...
    if (Math.random() > ((Math.cos(((1 - enemyCount / 50) * Math.PI / 2 + Math.PI / 2)) * .01 + .99))) {
        lastSpawn = new Date().getTime();
        entities.push(newEnemy());
        console.log("spawned" + enemyCount);
    }
    var pos = player.mesh.position.clone();
    pos.sub(player.v.clone().multiplyScalar(30 * (player.inside ? 1.8 : 1))).add(pos.clone().normalize().multiplyScalar((player.inside ? -.9 : 1) * 50));
